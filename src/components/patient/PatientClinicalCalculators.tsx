...
export const PatientClinicalCalculators: React.FC<Props> = ({ patient, onUpdatePatient }) => {
  ...
  const [form, setForm] = useState<Record<string, number | boolean>>({
    ...
    timiAge65: num(patient.age) >= 65, timiRiskFactors: 0, timiKnownCAD: false, timiSevereAngina: false, timiAspirin: false, timiSTDeviation: false, timiBiomarkers: false,
    ...
    heartHistory: false, heartECG: 0, heartAge: num(patient.age), heartRisk: 0, heartTroponin: 0,
  ...
  const result = useMemo(() => {
    ...
    if (calculator === 'grace') {
      ...
      if (form.timiAge65) score++; score += Math.min(3, num(form.timiRiskFactors)); if (form.timiKnownCAD) score++; if (form.timiSevereAngina) score++; if (form.timiAspirin) score++; if (form.timiSTDeviation) score++; if (form.timiBiomarkers) score++;
      ...
      score = num(form.heartHistory) + num(form.heartECG) + num(form.heartAge) + num(form.heartRisk) + num(form.heartTroponin);
  ...
  const Input = ({ field, label, step = '1' }: { field: string; label: string; step?: string }) => <label className="text-xs block">{label}<input type="number" step={step} value={Number(form[field])} onChange={e => set(field, Number(e.target.value))} className="mt-1 w-full px-2.5 py-2 rounded-lg border bg-slate-50 dark:bg-slate-800 dark:border-slate-700" /></label>;
  const SelectScore = ({ field, label, options }: { field: string; label: string; options: string[] }) => <label className="text-xs block">{label}<select value={Number(form[field])} onChange={e => set(field, Number(e.target.value))} className="mt-1 w-full px-2.5 py-2 rounded-lg border bg-slate-50 dark:bg-slate-800 dark:border-slate-700">{options.map((o, i) => <option key={o} value={i}>{o}</option>)}</select></label>;
  ...
  const renderInputs = () => {
    ...
    if (calculator === 'timi') return <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><Toggle label="Age ≥65 years" field="timiAge65" /><Toggle label="Known CAD ≥50% stenosis" field="timiKnownCAD" /><Toggle label="Severe angina ≥2 episodes/24h" field="timiSevereAngina" /><Toggle label="Aspirin use in past 7 days" field="timiAspirin" /><Toggle label="ST deviation ≥0.5 mm" field="timiSTDeviation" /><Toggle label="Elevated cardiac biomarkers" field="timiBiomarkers" /><Input field="timiRiskFactors" label="Number of CAD risk factors (0–6)" /></div>;
    if (calculator === 'chadsvasc') return <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><Toggle label="Congestive heart failure / LV dysfunction" field="chf" /><Toggle label="Hypertension" field="hypertension" /><Toggle label="Diabetes mellitus" field="diabetes" /><Toggle label="Prior stroke / TIA / thromboembolism" field="stroke" /><Toggle label="Vascular disease (MI/PAD/aortic plaque)" field="vascular" /><div className="text-xs text-slate-500">Age and sex are taken from the patient file: {patient.age} / {patient.gender}.</div></div>;
    if (calculator === 'hasbled') return <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><Toggle label="Abnormal renal function" field="abnormalRenal" /><Toggle label="Abnormal liver function" field="abnormalLiver" /><Toggle label="Prior stroke" field="stroke" /><Toggle label="Prior major bleeding / predisposition" field="bleeding" /><Toggle label="Labile INR" field="labileINR" /><Toggle label="Drugs / alcohol predisposing to bleeding" field="drugsAlcohol" /><div className="text-xs text-slate-500">Age &gt;65 is counted automatically.</div></div>;
    if (calculator === 'heart') return <div className="grid grid-cols-2 sm:grid-cols-3 gap-3"><SelectScore field="heartHistory" label="History" options={['Slightly suspicious (0)','Moderately suspicious (1)','Highly suspicious (2)']} /><SelectScore field="heartECG" label="ECG" options={['Normal (0)','Nonspecific repolarization (1)','Significant ST depression (2)']} /><SelectScore field="heartAge" label="Age points" options={['<45 years (0)','45–64 years (1)','≥65 years (2)']} /><SelectScore field="heartRisk" label="Risk factors" options={['None (0)','1–2 risk factors (1)','≥3 risk factors or known ASCVD (2)']} /><SelectScore field="heartTroponin" label="Troponin" options={['≤normal limit (0)','1–3× normal limit (1)','>3× normal limit (2)']} /></div>;
    if (calculator === 'wellspe') return <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><Toggle label="Clinical signs of DVT (+3)" field="peDVT" /><Toggle label="PE most likely diagnosis (+3)" field="peLikely" /><Toggle label="Immobilization ≥3 days / surgery within 4 weeks (+1.5)" field="peImmobilization" /><Toggle label="Previous DVT/PE (+1.5)" field="pePreviousVTE" /><Toggle label="Hemoptysis (+1)" field="peHemoptysis" /><Toggle label="Malignancy (+1)" field="peCancer" /><Input field="heartRate" label="Heart rate" /></div>;
    ...
    if (calculator === 'spesi') return <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><div className="text-xs text-slate-500">Age: {patient.age}</div><Toggle label="History of cancer" field="spesiCancer" /><Toggle label="Chronic cardiopulmonary disease" field="spesiCardiopulmonary" /><Input field="heartRate" label="Heart rate" /><Input field="spesiSBP" label="Systolic BP" /><Input field="spesiSpo2" label="O₂ saturation" /></div>;
  ...
  return <div className="space-y-4">
    <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
      <div className="flex items-center justify-between gap-3 mb-4"><div><h3 className="text-base font-bold flex items-center gap-2"><Calculator size={18} className="text-emerald-500" /> Clinical Calculators</h3><p className="text-xs text-slate-500">Patient-specific scores with patient demographics and latest data pre-filled when available.</p></div><span className="text-[10px] px-2 py-1 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200">Patient-linked</span></div>
      <div className="flex gap-2 mb-4"><select value={calculator} onChange={e => setCalculator(e.target.value as CalculatorKey)} className="w-full px-3 py-2 rounded-xl border bg-slate-50 dark:bg-slate-800 dark:border-slate-700 text-sm font-semibold"><optgroup label="Cardiology">{['grace','timi','chadsvasc','hasbled','heart'].map(k => <option key={k} value={k}>{calculatorLabels[k as CalculatorKey]}</option>)}</optgroup><optgroup label="VTE / Emergency">{['wellspe','wellsdvt','fourts','spesi'].map(k => <option key={k} value={k}>{calculatorLabels[k as CalculatorKey]}</option>)}</optgroup><optgroup label="ICU / Critical Care">{['qsofa','sofa','shockindex','map'].map(k => <option key={k} value={k}>{calculatorLabels[k as CalculatorKey]}</option>)}</optgroup><optgroup label="Clinical / Laboratory">{['crcl','aniongap','correctedcalcium'].map(k => <option key={k} value={k}>{calculatorLabels[k as CalculatorKey]}</option>)}</optgroup></select></div>
      ...
      {renderInputs()}
      <div className="mt-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700 flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs text-slate-500">Result</div><div className="text-2xl font-black text-slate-900 dark:text-white">{Number(result.score.toFixed(2))}</div><div className="text-xs font-semibold text-emerald-600">{result.interpretation}</div></div><button onClick={saveResult} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-600 text-white text-xs font-bold"><Save size={14} /> Save to Patient File</button></div>
    </div>
    <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800"><h4 className="text-sm font-bold flex items-center gap-2 mb-3"><History size={16} className="text-emerald-500" /> Calculation History</h4>{(patient.clinicalCalculations || []).length === 0 ? <p className="text-xs text-slate-500">No saved calculations yet.</p> : <div className="space-y-2">{(patient.clinicalCalculations || []).map(item => <div key={item.id} className="flex items-center justify-between gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/70"><div><div className="text-xs font-bold">{item.calculator}: <span className="text-emerald-600">{item.score}</span></div><div className="text-[11px] text-slate-500 flex items-center gap-1"><Clock size={11} /> {new Date(item.timestamp).toLocaleString()}</div><div className="text-[11px] text-slate-600 dark:text-slate-300">{item.interpretation}</div></div><button onClick={() => removeResult(item.id)} className="p-1.5 rounded-lg text-slate-400 hover:text-rose-600"><Trash2 size={14} /></button></div>)}</div>}</div>
  </div>;
};
