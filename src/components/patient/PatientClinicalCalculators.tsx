...
export const PatientClinicalCalculators: React.FC<Props> = ({ patient, onUpdatePatient }) => {
  ...
  const Input = ({ field, label, step = '1' }: { field: string; label: string; step?: string }) => <label className="text-xs block">{label}<input type="number" inputMode="decimal" step={step} value={form[field] === undefined || form[field] === '' ? '' : String(form[field])} onChange={e => set(field, e.target.value)} className="mt-1 w-full px-2.5 py-2 rounded-lg border bg-slate-50 dark:bg-slate-800 dark:border-slate-700 touch-manipulation" /></label>;
