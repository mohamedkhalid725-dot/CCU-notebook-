...
export const PatientClinicalCalculators: React.FC<Props> = ({ patient, onUpdatePatient }) => {
  ...
  const [form, setForm] = useState<Record<string, number | string | boolean>>({
    age: num(patient.age), genderFemale: patient.gender === 'Female',
    chadsAge: num(patient.age), chadsFemale: patient.gender === 'Female',
    hasbledAge: num(patient.age), spesiAge: num(patient.age), crclFemale: patient.gender === 'Female',
    heartRate: num(latestVital?.hr), systolicBP: num(latestVital?.bpSystolic), diastolicBP: num(latestVital?.bpDiastolic), creatinine: num(latestLab?.creatinine), killip: 1,
  ...
  const result = useMemo(() => {
    ...
    if (calculator === 'grace') {
      ...
      const age = num(form.chadsAge); if (form.chf) score++; if (form.hypertension) score++; if (age >= 65 && age <= 74) score++; if (age >= 75) score += 2; if (form.diabetes) score++; if (form.stroke) score += 2; if (form.vascular) score++; if (form.chadsFemale) score++;
      ...
      if (form.abnormalRenal) score++; if (form.abnormalLiver) score++; if (form.stroke) score++; if (form.bleeding) score++; if (form.labileINR) score++; if (num(form.hasbledAge) > 65) score++; if (form.drugsAlcohol) score++;
      ...
      score = num(form.spesiAge) > 80 ? 1 : 0; if (form.spesiCancer) score++; if (form.spesiCardiopulmonary) score++; if (num(form.heartRate) >= 110) score++; if (num(form.spesiSBP) < 100) score++; if (num(form.spesiSpo2) < 90) score++;
      ...
      score = ((140 - age) * weight) / (72 * creat) * (form.crclFemale ? 0.85 : 1);
  ...
  const stopTouch = (e: React.SyntheticEvent) => e.stopPropagation();
  const Toggle = ({ label, field }: { label: string; field: string }) => <label className="flex items-center gap-2 text-xs text-slate-700 dark:text-slate-300 cursor-pointer touch-manipulation"><input type="checkbox" checked={Boolean(form[field])} onChange={e => set(field, e.target.checked)} onPointerDown={stopTouch} onTouchStart={stopTouch} onClick={stopTouch} /><span>{label}</span></label>;
  const Input = ({ field, label, step = '1' }: { field: string; label: string; step?: string }) => {
    const isDecimal = step !== '1';
    return <label className="text-xs block select-text">{label}<input type="text" inputMode={isDecimal ? 'decimal' : 'numeric'} pattern={isDecimal ? '[0-9]*[.,]?[0-9]*' : '[0-9]*'} autoComplete="off" autoCorrect="off" spellCheck={false} enterKeyHint="done" readOnly={false} disabled={false} value={form[field] === undefined || form[field] === '' ? '' : String(form[field])} onFocus={e => e.currentTarget.select()} onPointerDown={stopTouch} onTouchStart={stopTouch} onClick={stopTouch} onInput={e => set(field, e.currentTarget.value)} onChange={e => set(field, e.target.value)} className="mt-1 w-full min-h-11 px-2.5 py-2 rounded-lg border bg-slate-50 dark:bg-slate-800 dark:border-slate-700 touch-manipulation select-text" style={{ WebkitUserSelect: 'text', userSelect: 'text', touchAction: 'manipulation' }} /></label>;
  };
  const SelectScore = ({ field, label, options }: { field: string; label: string; options: string[] }) => <label className="text-xs block">{label}<select value={Number(form[field])} onChange={e => set(field, Number(e.target.value))} onPointerDown={stopTouch} onTouchStart={stopTouch} onClick={stopTouch} className="mt-1 w-full min-h-11 px-2.5 py-2 rounded-lg border bg-slate-50 dark:bg-slate-800 dark:border-slate-700 touch-manipulation">{options.map((o, i) => <option key={o} value={i}>{o}</option>)}</select></label>;
  ...
  const renderInputs = () => {
    ...
    if (calculator === 'chadsvasc') return <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><Input field="chadsAge" label="Age" /><Toggle label="Female sex" field="chadsFemale" /><Toggle label="Congestive heart failure / LV dysfunction" field="chf" /><Toggle label="Hypertension" field="hypertension" /><Toggle label="Diabetes mellitus" field="diabetes" /><Toggle label="Prior stroke / TIA / thromboembolism" field="stroke" /><Toggle label="Vascular disease (MI/PAD/aortic plaque)" field="vascular" /></div>;
    if (calculator === 'hasbled') return <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><Input field="hasbledAge" label="Age" /><Toggle label="Abnormal renal function" field="abnormalRenal" /><Toggle label="Abnormal liver function" field="abnormalLiver" /><Toggle label="Prior stroke" field="stroke" /><Toggle label="Prior major bleeding / predisposition" field="bleeding" /><Toggle label="Labile INR" field="labileINR" /><Toggle label="Drugs / alcohol predisposing to bleeding" field="drugsAlcohol" /></div>;
    ...
    if (calculator === 'spesi') return <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><Input field="spesiAge" label="Age" /><Toggle label="History of cancer" field="spesiCancer" /><Toggle label="Chronic cardiopulmonary disease" field="spesiCardiopulmonary" /><Input field="heartRate" label="Heart rate" /><Input field="spesiSBP" label="Systolic BP" /><Input field="spesiSpo2" label="O₂ saturation" /></div>;
    ...
    if (calculator === 'crcl') return <div className="grid grid-cols-2 sm:grid-cols-4 gap-3"><Input field="age" label="Age" /><Input field="weight" label="Weight kg" /><Input field="creatinine" label="Serum creatinine mg/dL" step="0.1" /><Toggle label="Female sex (×0.85)" field="crclFemale" /></div>;
  ...
  return <div className="space-y-4">
    <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
      <div className="flex items-center justify-between gap-3 mb-4"><div><h3 className="text-base font-bold flex items-center gap-2"><Calculator size={18} className="text-emerald-500" /> Clinical Calculators</h3><p className="text-xs text-slate-500">Patient-linked values are pre-filled only as a starting point. Every calculator field remains manually editable and independent.</p></div><span className="text-[10px] px-2 py-1 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200">Editable</span></div>
      <div className="flex gap-2 mb-4"><select value={calculator} onChange={e => setCalculator(e.target.value as CalculatorKey)} onPointerDown={stopTouch} onTouchStart={stopTouch} onClick={stopTouch} className="w-full min-h-11 px-3 py-2 rounded-xl border bg-slate-50 dark:bg-slate-800 dark:border-slate-700 text-sm font-semibold touch-manipulation"><optgroup label="Cardiology">{['grace','timi','chadsvasc','hasbled','heart'].map(k => <option key={k} value={k}>{calculatorLabels[k as CalculatorKey]}</option>)}</optgroup><optgroup label="VTE / Emergency">{['wellspe','wellsdvt','fourts','spesi'].map(k => <option key={k} value={k}>{calculatorLabels[k as CalculatorKey]}</option>)}</optgroup><optgroup label="ICU / Critical Care">{['qsofa','sofa','shockindex','map'].map(k => <option key={k} value={k}>{calculatorLabels[k as CalculatorKey]}</option>)}</optgroup><optgroup label="Clinical / Laboratory">{['crcl','aniongap','correctedcalcium'].map(k => <option key={k} value={k}>{calculatorLabels[k as CalculatorKey]}</option>)}</optgroup></select></div>
