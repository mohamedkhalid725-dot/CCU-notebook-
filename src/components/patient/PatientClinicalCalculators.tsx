...
export const PatientClinicalCalculators: React.FC<Props> = ({ patient, onUpdatePatient }) => {
  ...
  const Toggle = ({ label, field }: { label: string; field: string }) => <label className="flex items-center gap-2 text-xs text-slate-700 dark:text-slate-300 cursor-pointer touch-manipulation"><input type="checkbox" checked={Boolean(form[field])} onChange={e => set(field, e.target.checked)} /><span>{label}</span></label>;
  const Input = ({ field, label, step = '1' }: { field: string; label: string; step?: string }) => {
    const isDecimal = step !== '1';
    return <label className="text-xs block select-text">{label}<input type="text" inputMode={isDecimal ? 'decimal' : 'numeric'} pattern={isDecimal ? '[0-9]*[.,]?[0-9]*' : '[0-9]*'} autoComplete="off" autoCorrect="off" spellCheck={false} enterKeyHint="done" readOnly={false} disabled={false} value={form[field] === undefined || form[field] === '' ? '' : String(form[field])} onInput={e => set(field, e.currentTarget.value)} onChange={e => set(field, e.target.value)} className="mt-1 w-full min-h-11 px-2.5 py-2 rounded-lg border bg-slate-50 dark:bg-slate-800 dark:border-slate-700 select-text" style={{ WebkitUserSelect: 'text', userSelect: 'text' }} /></label>;
  };
