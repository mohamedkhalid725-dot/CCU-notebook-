...
export const PatientClinicalCalculators: React.FC<Props> = ({ patient, onUpdatePatient }) => {
  ...
  const Input = ({ field, label, step = '1' }: { field: string; label: string; step?: string }) => <label className="text-xs block">{label}<input type="text" inputMode="decimal" autoComplete="off" enterKeyHint="done" value={form[field] === undefined || form[field] === '' ? '' : String(form[field])} onFocus={e => e.currentTarget.select()} onChange={e => set(field, e.target.value)} className="mt-1 w-full min-h-11 px-2.5 py-2 rounded-lg border bg-slate-50 dark:bg-slate-800 dark:border-slate-700 touch-manipulation select-text" /></label>;
  const SelectScore = ({ field, label, options }: { field: string; label: string; options: string[] }) => <label className="text-xs block">{label}<select value={Number(form[field])} onChange={e => set(field, Number(e.target.value))} className="mt-1 w-full px-2.5 py-2 rounded-lg border bg-slate-50 dark:bg-slate-800 dark:border-slate-700 touch-manipulation">{options.map((o, i) => <option key={o} value={i}>{o}</option>)}</select></label>;
