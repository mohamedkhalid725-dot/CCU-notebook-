    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 pt-[max(0.75rem,env(safe-area-inset-top))] pb-[max(0.75rem,env(safe-area-inset-bottom))] bg-black/60 backdrop-blur-sm overflow-hidden animate-in fade-in duration-200">
      <div className="relative flex flex-col w-full max-w-6xl h-[94vh] max-h-[950px] bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden text-slate-900 dark:text-slate-100">
        <header className="flex flex-wrap items-center justify-between gap-3 px-5 py-3.5 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shrink-0">
          <div className="flex items-center gap-3 min-w-0"><div className="flex items-center justify-center w-10 h-10 rounded-2xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold shrink-0"><BedDouble size={20} /></div><div className="min-w-0"><div className="flex items-center gap-2 flex-wrap"><h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white truncate">{patient.name}</h2>{patient.gender && <span className="text-xs text-slate-400 font-medium">({patient.gender}, {patient.age || '—'}y)</span>}<span className="text-xs px-2.5 py-0.5 rounded-full font-bold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">{patient.bedName ? patient.bedName : `Bed ${patient.bedNumber}`}</span>{patient.mrn && <span className="text-xs text-slate-400 font-mono hidden sm:inline">MRN: {patient.mrn}</span>}</div>{(patient.primaryDiagnosis || patient.diagnosis) && <p className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-md">{patient.primaryDiagnosis || patient.diagnosis}</p>}</div></div>
          <div className="flex items-center gap-2 ml-auto"><select value={patient.status} onChange={(e) => handleStatusChange(e.target.value as any)} className={`text-xs font-bold uppercase rounded-xl px-2.5 py-1.5 border cursor-pointer ${getStatusBadgeColor(patient.status)}`}><option value="stable" className="dark:bg-slate-900">Stable</option><option value="guarded" className="dark:bg-slate-900">Guarded</option><option value="deteriorating" className="dark:bg-slate-900">Deteriorating</option><option value="critical" className="dark:bg-slate-900">Critical</option><option value="post-op" className="dark:bg-slate-900">Post-Op</option><option value="discharged" className="dark:bg-slate-900">Discharged</option></select><button onClick={() => onPrintPatient(patient)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/60 hover:bg-emerald-600 hover:text-white dark:hover:bg-emerald-600 dark:hover:text-white transition shadow-xs" title="Export Clinical File to PDF"><FileDown size={14} /><span className="hidden sm:inline">Export PDF</span></button>{patient.status !== 'discharged' ? <button onClick={() => setShowDischargeConfirm(true)} className="p-2 rounded-xl text-slate-500 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-950/40 transition" title="Discharge Patient"><LogOut size={16} /></button> : <button onClick={() => onReadmitPatient(patient)} className="p-2 rounded-xl text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 transition" title="Readmit Patient"><RotateCcw size={16} /></button>}<button onClick={() => setShowDeleteConfirm(true)} className="p-2 rounded-xl text-slate-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40 transition" title="Delete Record"><Trash2 size={16} /></button><button onClick={onClose} className="p-2 rounded-xl text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition ml-1"><X size={18} /></button></div>
        </header>
